...
var widthElement = null;				// the dialog width element
var widthTypeElement = null;			// the dialog width type element
var heightElement = null;				// the dialog height element
var heightTypeElement = null;			// the dialog height type element
...
var RJ_InsertCodeDialogue =
{
	...
	parseInitialData : function()
	{
		...
		xmlhttp.open('POST', '../../../../../lib/editor/tinymce/plugins/rj_insertcode/php/rj_get_lang_select.php', false);
		...
		if (codeRootElement != null)
		{
			...
			var widthNumber = parseInt(codeRootElement.style.width);
			if( !isNaN(widthNumber) )
			{
				widthElement.value = widthNumber + "";
				if( codeRootElement.style.width.indexOf('%') != -1 )
					selectOptionByValue( widthTypeElement, "percent" );
				else
					selectOptionByValue( widthTypeElement, "pixels" );
			}
			
			var heightNumber = parseInt(codeRootElement.style.height);
			if( !isNaN(heightNumber) )
			{
				heightElement.value = heightNumber + "";
				if( codeRootElement.style.height.indexOf('%') != -1 )
					selectOptionByValue( heightTypeElement, "percent" );
				else
					selectOptionByValue( heightTypeElement, "pixels" );
			}
	...
	insert : function()
	{
		...
		var divOverflow   = "auto";
		var divBorder     = "1px solid #054b6e"
		var divBackground = "#f8f8f8"
		var divWidth      = "auto";
		var divHeight     = "auto";
	
		// extract width	
		var widthNumber = parseInt(widthElement.value);
		if( !isNaN(widthNumber) )
		{
			if( widthTypeElement.value == "percent" )
			{
				divWidth = widthNumber + "%"
			}
			else if( widthTypeElement.value == "pixels" )
			{
				divWidth = widthNumber + "px"
			}
		}

		// extract height
		var heightNumber = parseInt(heightElement.value);
		if( !isNaN(heightNumber) )
		{
			if( heightTypeElement.value == "percent" )
			{
				divHeight = heightNumber + "%"
			}
			else if( heightTypeElement.value == "pixels" )
			{
				divHeight = heightNumber + "px"
			}
		}
		...
		var xmlhttp = new XMLHttpRequest();
		xmlhttp.open('POST', '../../../../../lib/editor/tinymce/plugins/rj_insertcode/php/rj_get_highlighted_code.php', false);
		xmlhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
		xmlhttp.send(urlParams);
		...
		if (xmlhttp.status==200)
		{
			...
			if ( codeRootElement == null )
			{
				...
				tinyMCEPopup.execCommand(
						...
						'<div class=\"rj_insertcode\">'
						   + '<div class=\"' + languageClassPrefix + codeType
						   + '\" style=\"overflow: ' + divOverflow
						   + '; width: ' + divWidth
						   + '; height: ' + divHeight
						   + '; border: ' + divBorder
			...
			else
			{
				...
				codeRootElement.style.overflow   = divOverflow;
				
				codeRootElement.style.width		= divWidth;
				codeRootElement.style.height	= divHeight;
				
				codeRootElement.style.border     = divBorder;
				codeRootElement.style.background = divBackground;
	...
	resizeInputs : function()
	{
		if (!tinymce.isIE)
		{
			 contentHeight	= self.innerHeight - (265);
		}
		else
		{
			 contentHeight	= document.body.clientHeight - (265);
		}
	...
	init : function()
	{
		...
		widthElement				= document.getElementById('rj_width');		
		heightElement				= document.getElementById('rj_height');		
		widthTypeElement			= document.getElementById('rj_widthType');		
		heightTypeElement			= document.getElementById('rj_heightType');		
